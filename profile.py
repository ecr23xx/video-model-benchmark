...
def match_path(self, paths, path):
    if paths is not None and path in paths:
        return True

    if paths is None and leaf:
        return True

    for pp in paths:
        match = True
        for i, p in reversed(list(enumerate(pp))):
            if i >= len(path):
                match = False
                break
            if p != path[-i-1]:
...
def traces_to_display(traces, trace_events, show_events=False, paths=None):
                ...
                if show_events:
                    for event in events:
                        current_tree[name][event.name] = {
                            None: Measure(
                                sum([
                                    e.self_cpu_time_total for e in events if e.name == event.name]),
                                sum([
                                    e.cpu_time_total for e in events if e.name == event.name]),
                                sum([
                                    e.cuda_time_total for e in events if e.name == event.name]),
    ...
    # construct the table
    heading = ("Module", "Self CPU total", "CPU total",
               "CUDA total", "Occurrences")
